...
*/
//static 
void SamplingPointCloud::Uniform( PointCloud& src, PointCloud& dst, SamplingParam param)
{
    int size = src.size();
    //--------------
    ...
    for(itr_x = cube.begin(); itr_x != cube.end(); itr_x++ ){
        for(itr_y = itr_x->begin(); itr_y != itr_x->end(); itr_y++ ){
            for(itr_z = itr_y->begin(); itr_z != itr_y->end(); itr_z++ ){
                int idx = (*itr_z);
                if( idx != -1 ){
                    dst.points[i] =  src.points[idx];
                    dst.normals[i] =  src.normals[idx];
                    i++;
                }
            }
        }
...
*/
//static 
void SamplingPointCloud::SetMethod(SamplingMethod method, SamplingParam param)
{
    curr_method = method;
    curr_param = param;
}
...
*/
//static 
void SamplingPointCloud::Sample(PointCloud& src, PointCloud& dst, bool verbose)
{
    g_verbose = verbose;
    switch(curr_method){
        case RAW:
